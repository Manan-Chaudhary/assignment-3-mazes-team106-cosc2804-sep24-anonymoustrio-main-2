...
#include <thread>
#include <chrono>
#include <set>      // Ensure this header is included for std::set
#include <map>      // Ensure this header is included for std::map
#include <queue>
...
typedef void (*StateFunction)(mcpp::MinecraftConnection&, const std::vector<std::string>&, int, int, int, bool);
...
// Map to associate states with corresponding functions
std::map<States, StateFunction> stateFunctions = {
    {ST_Main, [](mcpp::MinecraftConnection& mc, const std::vector<std::string>& maze, int x, int y, int z, bool mode) {
        printMainMenu();
        int choice;
        std::cin >> choice;
        if (std::cin.fail() || choice < 1 || choice > 5) {
            ...
            if (choice == 5) {
                stateFunctions[ST_Exit](mc, maze, x, y, z, mode);
            } else {
                stateFunctions[static_cast<States>(choice)](mc, maze, x, y, z, mode);
            }
        }
    }},
    {ST_GetMaze, [](mcpp::MinecraftConnection& mc, const std::vector<std::string>& maze, int x, int y, int z, bool mode) {
        ...
        if (std::cin.fail() || choice < 1 || choice > 3) {
            ...
            if (choice == 3) {
                stateFunctions[ST_Main](mc, maze, x, y, z, mode);
            } else {
                stateFunctions[static_cast<States>(ST_BuildMaze - 1 + choice)](mc, maze, x, y, z, mode);
            }
        }
    }},
    {ST_BuildMaze, [](mcpp::MinecraftConnection& mc, const std::vector<std::string>& maze, int x, int y, int z, bool mode) {
    ...
    {ST_SolveMaze, [](mcpp::MinecraftConnection& mc, const std::vector<std::string>& maze, int x, int y, int z, bool mode) {
        printSolveMazeMenu();
        int choice;
        std::cin >> choice;
        if (std::cin.fail() || choice < 1 || choice > 3) {
            ...
            if (choice == 3) {
                ...
                stateFunctions[static_cast<States>(ST_Creators - 1 + choice)](mc, maze, x, y, z, mode);
            }
        }
    }},
    {ST_Creators, [](mcpp::MinecraftConnection& mc, const std::vector<std::string>& maze, int x, int y, int z, bool mode) {
        printTeamInfo();
        stateFunctions[ST_Main](mc, maze, x, y, z, mode);
    }},
    {ST_Exit, [](mcpp::MinecraftConnection& mc, const std::vector<std::string>& maze, int x, int y, int z, bool mode) {
        cleanWorld(mc);
        printExitMessage();
        exit(EXIT_SUCCESS);
    }}
...
int main(int argc, char* argv[]) {
    ...
    while (true) {
        stateFunctions[curState](mc, currentMaze, mazeX, mazeY, mazeZ, mode);
    }
...
void generateRandomMaze(mcpp::MinecraftConnection& mc, bool mode) {
    ...
    if (done == "done") {
        ...
        std::vector<std::string> maze = generateMaze(length, width);
        std::cout << "Random Maze generated successfully" << std::endl;
        std::cout << "**Printing Maze**" << std::endl;
        std::cout << "BasePoint: (" << x << ", " << y << ", " << z << ")" << std::endl;
        std::cout << "Structure:" << std::endl;
        for (const auto& line : maze) {
